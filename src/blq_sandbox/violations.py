...
VIOLATION_PATTERNS: list[tuple[re.Pattern[str], str, str]] = [
    # Filesystem
    (re.compile(r"Read-only file system", re.IGNORECASE), "filesystem", "write to read-only filesystem"),
    (re.compile(r"cannot (?:touch|create|open)\b.*(?:Permission denied|Read-only)", re.IGNORECASE), "filesystem", "write blocked"),
